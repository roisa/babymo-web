...
// their activity area; this template wraps it with a printable A4 page
// (titled header, instructions, and footer).

// Page: 210 x 297 (A4) in user units (1 unit = 1 mm), with 16mm side margins.
export const PAGE = {
  width: 210,
  height: 297,
  marginX: 16,
...
export function getActivityRect() {
  ...
  const height = PAGE.height - y - PAGE.marginBottom - PAGE.footerHeight;
...
export function renderWorksheet({
  ...
  <!-- Footer -->
  <text x="${pw / 2}" y="${ph - 6}" font-size="3" fill="#8a90a8" text-anchor="middle">Made with Baby Mo · babymo.id</text>
</svg>`;
}
